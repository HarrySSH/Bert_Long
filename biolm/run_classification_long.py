...
from biolm.utils_classification_long import processors, stopping_metrics, Bert_long
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if args.output_mode == 'multilabel_classification':
                ...
                vec_list = []
                for i in range(3):
                    inputs = {"input_ids": batch[0][:,512 * i:512 * (i + 1)],
                              "attention_mask": batch[1][:,512 * i:512 * (i + 1)],
                              "token_type_ids":batch[2][:,512 * i:512 * (i + 1)]}
                    labels = batch[3]

                    vec_list.append(inputs)


                outputs = model(vec_list)
                labels = batch[3]
                # I try the Cross Entropy Loss
...
def evaluate_train(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        if args.output_mode == "classification":
            pred_logits = torch.sigmoid(torch.tensor(preds)).numpy()
        ...
        print('I want to see how the preds look like!')
        print(preds.shape)
        print(preds)
        result = compute_metrics(eval_task, preds, pred_logits, out_label_ids, examples)
        results_eva.update(result)
        results_eva['eval_loss'] = eval_loss
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        train_dataset, examples = load_and_cache_examples(args, eval_task, tokenizer, evaluate=False, return_examples=True)
        ...
        # Eval Training
        ################ } #######################################
        logger.info("***** Running evaluation for training set {} *****".format(prefix))
        logger.info("  Num examples = %d", len(train_dataset))
        ...
        results_train['eval_loss'] = eval_loss

        output_eval_file = os.path.join(eval_output_dir, prefix,
                                        "train_results.txt")
        with open(output_eval_file, "w") as writer:
            # with open('bioroberta_chemprot.txt', 'w') as writer:
            logger.info("***** Eval training results {} *****".format(prefix))
            for key in sorted(result.keys()):
                logger.info("  %s = %s", key, str(result[key]))
                writer.write("%s = %s\n" % (key, str(result[key])))
        ...
        print('I want to see how the preds look like! I mean before the transfrom')
        print(preds)
...
def main():
    ...
    model = Bert_long(_dir = args.model_name_or_path)
